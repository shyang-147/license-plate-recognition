...
report('plateFormat: 7 位 = 省简称 + 字母 + 5 位字母数字', ...
       F7.n == 7 && numel(F7.sets) == 7 && isequal(F7.sets{3}, F7.sets{6}) && ...
       all(ismember(F7.sets{3}, F7.sets{7})), ...
       sprintf('%s, 第 3 位允许 %d 个字符', F7.label, numel(F7.sets{3})));
report('plateFormat: 7 位末位额外允许制式后缀(警用)', ...
       any(F7.sets{7} == '警') && ~any(F7.sets{6} == '警'), ...
       sprintf('末位 %d 个字符, 第 6 位 %d 个', numel(F7.sets{7}), numel(F7.sets{6})));
