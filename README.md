...
**分辨率退化曲线**（第七轮，合成退化集 60 张，`bench_stage` 逐档归因）：
...
**先崩识别**（180 px 字符率就开始掉）、**再崩分割**（90~75 px 接棒）、**定位撑到 60 px**。
...
| 错在哪一步（定位/分割/识别） | `bench_stage`（或 `测试脚本/run_stage.m`） | 逐图归因到唯一一个阶段 + 按 `category` 分组；`run_stage` 覆盖 6 集（含两组退化集） |
