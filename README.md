...
| **误检（CCPD 40 张真实无车牌照片）** | **15/40 = 37.5%**（第九轮补齐白底通道的结构证据，原为 23/40 = 57.5%） | 未接入 |
...
挡不住汽车照片里的像车牌的矩形。**第九轮把它修掉三分之一（23/40 → 15/40）**：把结构证据
补齐到**白底通道**（实测它才是误检大头，放行了 10/23），代价为 0（定位命中仍 113/320）。
**第九轮还证伪了两条路线**：局部 / 自适应二值化（命中后字符率 15.3% → 4.3~8.0%，更差）
与彩色通道的颜色正向证据（两边判据分布几乎完全重叠）。详见
[`docs/改进记录.md`](docs/改进记录.md) 第八、九轮。
...
cd ../测试脚本 && matlab -batch "run_stage('第九轮')"   # 第八轮的凭证是 stage_第八轮_*.txt
```
