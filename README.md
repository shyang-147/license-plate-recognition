...
| 字符正确率（真实照片 3 张） | **45.5% (10/22)**（第四轮前 4.5%，第五轮白底前 27.3%） | 18.2% (4/22) |
...
- **bench/hard 上剩下的瓶颈只有斜拍和形近字符**：定位、几何校正、去外框、字符分割都已稳定
  ...
  `9↔X`、`6↔8` 这类形近字符上。
- **真实照片的瓶颈分三类，别混成一句「识别器不行」**：定位（第四轮修了选优、第五轮补了白底
  牌型）、**分割**（第五轮发现：车牌自带的边框线会把垂直投影连成一条，`real01` 的框一换高
  就崩）、分辨率（`real02` 单字 8×12 px）。CNN 只对得上最后一类里的「字形混淆」。
...
| **⑬ 模板导出脚本** | `项目代码/tools/export_templates.m`（新增） | 模板的唯一来源是 `templates.mat`，网页版 `site/src/templates.json` 由它导出（原有三组特征逐字节一致）；网页版 50 张图重测**严格 no-op** |
| **⑭ 闸门一致性（收尾）+ 白底误检测试** | `matlab/locatePlate.m`、`测试脚本/white_fp_eval.m`（新增） | ① `valid` 复算补上「边缘密度」，堵住"有颜色没字"的假块（纯色蓝块对照实验：补齐前 `valid=1` 放行、补齐后 `valid=0`）；② 7 张白底干扰图（白墙/白车身/白底广告牌/蓝天白云/白墙+黑线）误检由 1/7 收到 **0/7**。两项改动后 bench/images/hard/real2 **50 张逐图逐字节不变** |
...
「框切矮」反而是当前分割算法的保护色 —— 要同时保住框高和分割，得先修分割（见下一步 A）。

**⑭ 做的两件事**（都在 `结果记录/第五轮_*_实测证据.txt` 里留了证据）：
...
证据在 `结果记录/第五轮_噪声图误检_实测证据.txt`。

## 算法流程

| 步骤 | 文件（MATLAB） | 做法 |
|---|---|---|
| ① 车牌定位 | `locatePlate.m` | 蓝/绿/黄底色掩膜 + Sobel 边缘掩膜 + 形态学闭运算连成候选块；按长宽比、面积、底色占比、边缘密度、矩形度打分取最优；颜色/边缘两路都没找到真车牌时，再走一路**白底车牌**兜底（警用/军用/使领馆） |
...
- 只认**中国大陆**车牌。蓝/绿/黄底色走颜色掩膜；**白底警用车牌（`京A3454警`）第五轮已支持**
  （走「边缘掩膜 + 白色掩膜」的兜底通道，见上面的 ⑪），军用 / 使领馆牌没有实测过。
  ⚠️ **白底通道的主要风险是白色车身、白墙、白底广告牌**：本轮靠「框内必须有成排的字」
  （边缘密度 ≥ 0.02 + 相对对比度算出的墨迹占比 ≥ 0.05）+ 长宽比 ≤ 3.8 挡住，
  7 张合成干扰图（`测试脚本/white_fp_eval.m`）**误检 0**；
  但「白底 + 成排黑字 + 长宽比 3 左右」的牌子仍然分不出来，真实白车/白墙照片也没有测过。
- **纯随机噪声图会误检**（第四轮就有，第五轮未修）：680×1000 的纯噪声 8/8 会报出
  凭空捏造的车牌号。有结构的干扰图（蓝天/棋盘格/文字/路面/夜景）仍然挡得住。
  证据：`结果记录/第五轮_噪声图误检_实测证据.txt`。
...
- **真实照片要分两类看**：
  ...
  >   （**第五轮已修**：补上白底兜底通道后变成 `*A3453警`，序号与末位全对，见上面的 ⑪。）
  >
