...
/* ---------------- 识别侧低分辨率增强(第八轮) ----------------
   HOLE_W   孔洞数不一致的罚分(默认 0.04, 与 MATLAB recognizeChars.m 的
            'HoleWeight' 对齐): 低分辨率车牌放大后笔画被插值喂胖, "0" 的孔洞
            只剩 1~2 px, 得分与 "8" 精确打平(0.584 vs 0.584), 只能靠模板顺序
            取胜。孔洞数是拓扑量, 不受笔画粗细影响, 一罚就打破这种平局。
   USM_*    放大到 64 高之后的 USM 锐化(amount 0.6 / sigma 1.0, 与 MATLAB
            correctPlate.m 的 unsharp 对齐)。只对"放大"(s > 1.05)生效, 原图
            本来就有 64 高左右的完全不走这条路, 所以 bench/真实集零影响。 */
var HOLE_W = 0.04;
var HOLE_THR = 0.5;
var USM_AMOUNT = 0.6;
var USM_SIGMA = 1.0;

/* 孔洞数: 24x12 二值特征的拓扑量, 与 MATLAB holesOf 同口径 ——
   从边界灌水标出"外部背景", 剩下的背景连通域就是孔洞; 面积 < 2 px 的孔洞是
   插值毛刺, 不算(bwareaopen(bw,2))。 */
/* 注意 charFeature 的展开顺序是列优先(index = x*24 + y, 与 MATLAB 的 f(:)' 一致),
   而下面的灌水按行优先走, 所以先用 holesOf 转一遍, 否则连通关系全错。 */
function holesOf(f, thr) {
  var bw = new Uint8Array(288), x, y;
  for (x = 0; x < 12; x++) {
    for (y = 0; y < 24; y++) { bw[y * 12 + x] = f[x * 24 + y] > thr ? 1 : 0; }
  }
  return holesOfBin(bw, 12, 24);
}

function holesOfBin(bw, w, h) {
  var n = w * h, i, p, q, r, area, comp = 0;
  var outer = new Uint8Array(n), st = [];
  for (i = 0; i < n; i++) {
    if (bw[i]) { continue; }
    var x0 = i % w, y0 = (i - x0) / w;
    if (x0 === 0 || y0 === 0 || x0 === w - 1 || y0 === h - 1) { st.push(i); }
  }
  while (st.length) {
    p = st.pop();
    if (outer[p] || bw[p]) { continue; }
    outer[p] = 1;
  ...
  var seen = new Uint8Array(n);
  for (i = 0; i < n; i++) {
    if (bw[i] || outer[i] || seen[i]) { continue; }
    area = 0; q = [i]; seen[i] = 1;
    while (q.length) {
      r = q.pop(); area++;
      var rx = r % w, ry = (r - rx) / w;
      if (rx > 0 && !seen[r - 1] && !bw[r - 1] && !outer[r - 1]) { seen[r - 1] = 1; q.push(r - 1); }
      if (rx < w - 1 && !seen[r + 1] && !bw[r + 1] && !outer[r + 1]) { seen[r + 1] = 1; q.push(r + 1); }
      if (ry > 0 && !seen[r - w] && !bw[r - w] && !outer[r - w]) { seen[r - w] = 1; q.push(r - w); }
      if (ry < h - 1 && !seen[r + w] && !bw[r + w] && !outer[r + w]) { seen[r + w] = 1; q.push(r + w); }
    }
    if (area >= 2) { comp++; }
  }
  return comp;
...
/* USM 锐化 g + amount*(g - 高斯(g)), 5x5 sigma 高斯核 + replicate 边界,
   与 MATLAB imfilter(..., 'replicate') 同口径。只锐化灰度(分割/识别只用灰度)。 */
function unsharpF(g, w, h, amount, sigma) {
  if (!(amount > 0)) { return g; }
  var r = 2, dx, dy, t;
  var k = new Float32Array(25), s = 0;
  for (dy = -r; dy <= r; dy++) {
    for (dx = -r; dx <= r; dx++) {
      var v = Math.exp(-(dx * dx + dy * dy) / (2 * sigma * sigma));
      k[(dy + r) * 5 + (dx + r)] = v; s += v;
    }
  }
  for (t = 0; t < 25; t++) { k[t] /= s; }
  var out = new Float32Array(w * h);
  for (var y = 0; y < h; y++) {
    for (var x = 0; x < w; x++) {
      var acc = 0;
      for (dy = -r; dy <= r; dy++) {
        ...
        for (dx = -r; dx <= r; dx++) {
          ...
          acc += g[yy * w + xx] * k[(dy + r) * 5 + (dx + r)];
        }
      }
      var gv = g[y * w + x];
      var val = gv + amount * (gv - acc);
...
/* 连通域周长估计: 统计暴露在外的像素边(与 MATLAB regionprops 的 Perimeter 同量级) */
function compPerimeter(labels, id, w, h) {
  var p = 0, x, y, i;
  for (y = 0; y < h; y++) {
    for (x = 0; x < w; x++) {
      i = y * w + x;
      if (labels[i] !== id) { continue; }
      if (x === 0 || labels[i - 1] !== id) { p++; }
      if (x === w - 1 || labels[i + 1] !== id) { p++; }
      if (y === 0 || labels[i - w] !== id) { p++; }
      if (y === h - 1 || labels[i + w] !== id) { p++; }
    }
  }
  return p;
}

/* 整块删掉"贴着车牌外沿、又长又细"的连通域(车牌外框 / 白边残留)
   对应 segmentChars.m 的 removeEdgeSlivers, 三项同时成立才删:
   贴边(外接框触到外圈 ring+1 以内) + 最长边 >= minLen + 满足任一"纤细"判据:
     面积/最长边 <= maxThick  : 细长残段, 如被 JPEG 打断的下边框
     面积/周长   <= maxLineT  : 线条状, 如闭合的矩形外框白线
   闭合外框的面积/最长边约为真实线宽的 2 倍(长边、短边的面积都算进分子,
   却只除以一条长边), 实测 6.8 > 4.5, 会被第一条漏掉, 所以要补第二条。
   bw 已经清掉最外圈 ring 像素, 所以直接用 ring 判断"是否贴边"。 */
function removeEdgeSlivers(bw, w, h, ring, minLen, maxThick, maxLineT) {
  var res = label8(bw, w, h);
  var labels = res.labels, stats = res.stats;
  var drop = new Uint8Array(stats.length + 1), i;
  for (i = 0; i < stats.length; i++) {
    var st = stats[i];
    var x0 = st.minX + 1, x1 = st.maxX + 1;      /* 转成 1 基, 与 MATLAB 对齐 */
    var y0 = st.minY + 1, y1 = st.maxY + 1;
    if (!(x0 <= ring + 1 || y0 <= ring + 1 || x1 >= w - ring || y1 >= h - ring)) { continue; }
    var len = Math.max(st.maxX - st.minX + 1, st.maxY - st.minY + 1);
    if (len < minLen) { continue; }
    var perim = compPerimeter(labels, i + 1, w, h);
    if (st.area / len <= maxThick || st.area / Math.max(perim, 1) <= maxLineT) { drop[i + 1] = 1; }
  }
  for (i = 0; i < w * h; i++) { if (drop[labels[i]]) { bw[i] = 0; } }
  return bw;
}
...
function removeFrameLocal(bw, w, h) {
  ...
  /* 车牌外框一定紧贴裁剪边界, 而字符都在中部, 所以只删"贴着外沿、又长又细"的
     连通域(判据见 removeEdgeSlivers): 贴边 + 最长边 >= 0.30*H + 平均厚度 <= 4 px。
     数字 1 的竖线约 0.70*H, 但它在车牌中部, 不贴外沿, 不会被误删;
     字符笔画有 4~6 px 厚, 即使被裁到边也会因为"太粗"而放行。

     这里原来按"外圈窄带内清超长笔画"判断(垂直 0.60*H / 水平 0.35*W), 有两个毛病:
     一是实拍牌照的边框被 JPEG 与模糊打断成几段, 每段都短于阈值, 整条边框留下来
     (real01 的下边框断成 51 px 和 142 px 两段), 把每个字符的墨迹包围盒从 42 行
     撑到 58 行, 字符被纵向压扁, 匹配得分从 0.7 掉到 0.3;
     二是斜边框每行只占 1~2 px, 长度判据检不出来, 反而会因为某行笔画长就整行抹掉
     —— bench01 左下角留下的 1 px 宽、23 px 高的残端会被当成独立字符, 7 位牌切出 8 段。 */
  removeEdgeSlivers(out, w, h, ring, 0.30 * h, 4.5, 1.5);
...
function chooseCount(runs, proj) {
  /* 候选位数分两轮: 先只试 7 / 8(中国大陆单排车牌只有这两种制式: 普通牌
     7 位、新能源牌 8 位), 都不成立时才退回 6~8 全范围。
     实拍图里笔画粘连/断裂会让原始段数偏少, 而"中心间距最均匀"这个判据在
     段数越少时越容易偶然取到很小的变异系数 —— real01 就被选成了 6 段。
     先用制式先验收窄范围能挡住这类退化; 收窄后若一个都拟合不上
     (adjustToCount 会主动放弃而不是硬切), 再退回全范围, 不会漏掉特殊号牌。 */
  return tryCounts(runs, proj, 7, 8) || tryCounts(runs, proj, 6, 8) || runs;
}

/* TRYCOUNTS 在 lo~hi 里挑"相邻字符中心间距变异系数最小"的那个字数 */
function tryCounts(runs, proj, lo, hi) {
  var best = Infinity, bestRuns = null, n, i, r, d, m, v;
  for (n = lo; n <= hi; n++) {
    var copy = [];
...
function bestMatch(f, set, opts) {
  ...
  /* 二值化阈值只用来切"实测特征", 模板特征来自 0/1 图, 仍然用 0.5。
     这个值是各自流水线的调参常数, 两条流水线的最优值不同(都实测过):
       网页版(本文件, illumNormalize+Otsu):  0.35 -> hard 70.8%/93.7%, bench 90.0%/98.6%
                                              0.50 -> hard 62.5%/93.1%, bench 85.0%/97.9%
       MATLAB(adapthisteq+Otsu):             0.50 -> hard 66.7%/83.3%, bench 95.0%/99.3%
                                              0.35 -> hard 50.0%/80.5%, bench 95.0%/99.3%
     机制: 降阈值会把笔画喂胖(MATLAB 侧"3"变"8"、"0"变"9"), 但本文件的
     降采样/归一化更"细瘦", 需要松一点才能保住细笔画。改这个值前请重跑两边的
     run_js.mjs 与 run_all, 不要只调一边。
     注: 上面四行是第八轮识别侧增强**之前**扫出来的口径(当时 bench 90.0%/98.6%);
     加了孔洞罚分 + USM 之后两版都涨了(本文件 bench 95.0%/99.3%), 结论没变: 阈值仍是
     本文件 0.35 / MATLAB 0.5。第八轮之后的对照见 site/README.md 的实测表。 */
  for (i = 0; i < n; i++) { if (f[i] > 0.35) { fb[i] = 1; nf++; } meanF += f[i]; }
  meanF /= n;
  var fa = new Float32Array(n), acc = 0;
  for (i = 0; i < n; i++) { var d = f[i] - meanF; fa[i] = d; acc += d * d; }
  var nfNorm = Math.sqrt(acc);
  /* 查询侧孔洞数: 与 MATLAB holesOf 同口径, 单独按 0.5 二值(不是上面 IoU 用的阈值,
     两边最优阈值不同, 见上一段注释), 否则查询/模板的孔洞数不同口径, 反而罚错。 */
  var hq = holesOf(f, HOLE_THR);
  ...
  for (k = 0; k < set.items.length; k++) {
    ...
    if (HOLE_W > 0 && it.holes !== hq) { sc -= HOLE_W; }
...
function recognizeChars(charImages, inkBoxes) {
  ...
  for (var k = 0; k < n; k++) {
    ...
    /* 汉字之后的位置上, 如果这一段墨迹特别细长, 基本只可能是数字 1(车牌上唯一
       又窄又高的字符, 字母 I 已被排除)。归一化的 'fill' 模式会把细长笔画横向
       拉伸成实心块(实测用户照片里 AR=0.132 的 1 被拉宽后更像 8/4/0: 对模板 "1"
       只有 0.31, 而 8 拿到 0.51), 所以按宽度分两档:
         AR < 0.22 : 直接判定为 1 —— 逐图实测所有数据集里第 2 位起、AR < 0.22 的段 100% 是 1;
         0.22~0.30 : 只给一个很小的加分, 不强制 —— 新能源牌被纵向压缩时
    ...
    if (k >= 2 && inkBoxes && inkBoxes[k] && inkBoxes[k].ar > 0 && inkBoxes[k].ar < 0.30) {
      ...
      if (inkBoxes[k].ar < 0.22 || alt.score + 0.06 > r.score) { r = alt; }
    }
    var lb = r.label;
...
function run(imgData, options) {
  ...
  if (crop.h >= 8 && Math.abs(64 / crop.h - 1) > 0.01) {
    ...
    upscaled = 64 / crop.h > 1.05;
  }
  T.normalize = Date.now() - t3;

  var t4 = Date.now();
  var pg = rgbaToGrayF(plateRGBA.data, plateRGBA.w, plateRGBA.h);
  if (upscaled) { pg = unsharpF(pg, plateRGBA.w, plateRGBA.h, USM_AMOUNT, USM_SIGMA); }
...
var API = {
  ...
  setHoleWeight: function (v) { HOLE_W = v; },
  setHoleThreshold: function (v) { HOLE_THR = v; },
  setUnsharp: function (a, s) { USM_AMOUNT = a; if (s) { USM_SIGMA = s; } },
  _internals: {
