...
function removeFrameLocal(bw, w, h) {
  ...
  /* 车牌外框一定紧贴裁剪边界, 而字符都在中部, 所以只在外圈窄带里清"超长直线":
       竖直方向: 外 6% 宽度的列中, 连续笔画超过 0.60*H 的判为左右边框;
       水平方向: 外 12% 高度的行中, 连续笔画超过 0.35*W 的判为上下边框。
     数字 1 的竖线约 0.70*H, 但它落在车牌中部, 不进外圈窄带, 因此不会被误删;
     字符最长的横画(如"三"的横)也只有约 0.13*W, 0.35*W 留了足够余量。

     原来这两个阈值是 0.70*H / 0.70*W, 在合成图上够用, 到真实照片上就漏了:
     实拍牌照的边框被 JPEG 与模糊打断成几段(real01 的下边框断成 51 px 和
     142 px 两段, 都短于 0.60*W 的核长), 整条边框残留下来, 把每个字符的墨迹
     包围盒从 42 行撑到 58 行, 字符被纵向压扁, 匹配得分从 0.7 掉到 0.3。 */
  var bandX = Math.max(2, Math.round(0.06 * w));
  var bandY = Math.max(2, Math.round(0.12 * h));
  var vThr = 0.60 * h, hThr = 0.35 * w;
